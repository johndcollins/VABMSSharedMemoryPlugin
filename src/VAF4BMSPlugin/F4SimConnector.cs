...
{
    public class F4SimConnector : IDisposable
    {
        ...
        public void GetValue(dynamic vaProxy)
        {
            if (vaProxy.Context == "F4_SM")
            {
                string value = vaProxy.GetText("F4_SM_VALUE");
                if (_simOutputs[value] != null)
                {
                    if (_simOutputs[value].GetType() == typeof(AnalogSimOutput))
                    {
                        vaProxy.SetDecimal(value, Convert.ToDecimal(((AnalogSimOutput)_simOutputs[value]).State));
                    }
                    else if (_simOutputs[value].GetType() == typeof(DigitalSimOutput))
                    {
                        vaProxy.SetBoolean(value, ((DigitalSimOutput)_simOutputs[value]).State);
                    }
                    else if (_simOutputs[value].GetType() == typeof(TextSimOutput))
                    {
                        vaProxy.SetText(value, ((TextSimOutput)_simOutputs[value]).State);
                    }
                }
            }
        }
        ...
        private static void UpdateHsiData(FlightData flightData, out float courseDeviationDecimalDegrees, out float deviationLimitDecimalDegrees)
        {
            ...
            courseDeviationDecimalDegrees = courseDeviationDecimalDegrees < -90
                ? Util.AngleDelta(Math.Abs(courseDeviationDecimalDegrees), 180) % 180
                : (courseDeviationDecimalDegrees > 90 ? -Util.AngleDelta(courseDeviationDecimalDegrees, 180) % 180 : -courseDeviationDecimalDegrees);
        ...
        private static void SetOutput(AnalogSignal signal, double newVal)
        {
            ...
            var delta = signal.IsAngle && signal.MinValue == -180 && signal.MaxValue == 180
                ? Util.AngleDelta((float)(currentState.Value + 180), (float)(newVal + 180))
        ...
        private void UpdateSimOutputs()
        {
            ...
            //Parallel.ForEach(subscribedSimOutputs, simOutput => UpdateSimOutput(courseDeviationDegrees, deviationLimitDegrees, simOutput));
            Parallel.ForEach(_simOutputs.Values, simOutput => UpdateSimOutput(courseDeviationDegrees, deviationLimitDegrees, simOutput));
        }

        private void UpdateSimOutput(float courseDeviationDegrees, float deviationLimitDegrees, ISimOutput output)
        {
            ...
            switch (simOutput)
            {
                ...
                case F4SimOutputs.COMPASS__MAGNETIC_HEADING_DEGREES:
                    SetOutput((AnalogSignal)output, (360 + _lastFlightData.yaw / RADIANS_PER_DEGREE) % 360);
                    break;
